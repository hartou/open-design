...
function truncateForTranscript(content: string): string {
  ...
  return `${content.slice(0, MAX_TRANSCRIPT_MESSAGE_CHARS)}\n\n[ArrowFlow Design truncated ${omitted} chars from this prior message before sending it to the agent. Full content remains in persisted history.]`;
}
...
function buildPriorRunContextWarning(history: ChatMessage[]): string | null {
  ...
  return [
    '## context warning',
    `ArrowFlow Design detected ${notes.join(', ')}.`,
